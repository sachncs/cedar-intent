...
The version is declared once in ``cedar_intent.__version__`` and read at
...
import cedar_intent
from cedar_intent import __version__
...
def test_version_is_pep_440() -> None:
    assert _PEP_440.match(__version__), (
        f"cedar_intent.__version__={__version__!r} is not a PEP 440 release"
    )


def test_version_exposed_on_module() -> None:
    assert cedar_intent.__version__ == __version__


def test_pyproject_dynamic_version_attr_matches() -> None:
    """The build backend reads the version from ``cedar_intent.__version__``."""
    pyproject = (Path(__file__).resolve().parent.parent / "pyproject.toml").read_text()
    assert "version =" in pyproject
    assert "cedar_intent.__version__" in pyproject
